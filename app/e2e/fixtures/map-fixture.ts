 ...
 */
export async function getRegionScreenshot(
  page: Page,
  x: number,
  y: number,
  size = 50
 ...
 */
export async function navigateToNewMap(
  page: Page
): Promise<{success: boolean; skipReason?: string; documentId?: string}> {
  // Navigate to places
 ...
 */
export async function navigateToPlaces(
  page: Page
): Promise<{success: boolean; placeCount: number}> {
  await page.goto(testUrls.places);
  await page.waitForLoadState('domcontentloaded');
