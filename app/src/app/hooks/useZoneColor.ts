...
import {getCoiCommunityColor} from '@/app/utils/coiCommunities';
...
export const useZoneColorGetter = () => {
  ...
  return useCallback(
    (zone: NullableZone, fallbackColor = '#000000') => {
      ...
      if (mapMode === 'coi') {
        return getCoiCommunityColor(coiCommunities, zone, fallbackColor);
      }
      return colorScheme[(zone - 1) % colorScheme.length] ?? fallbackColor;
    },
    [coiCommunities, colorScheme, mapMode]
  );
};
