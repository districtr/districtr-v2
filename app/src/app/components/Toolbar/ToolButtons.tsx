...
export const ToolButtons: React.FC<{
  ...
  const renderTool = (tool: ActiveToolConfig, buttonStyle: React.CSSProperties) => {
    ...
    // need a shortcut tooltip — and only while Alt reveals shortcuts.
    const element =
      !isHistoryTool || !showHotkeyHints ? (
        button
      ) : (
    ...
    // HelpTip is a hover overlay (guide copy); the shortcut tooltip above is
    // alt-reveal only, so the two never fire on the same hover moment.
    return tool.helpKey ? (
