...
export const generateMetadata = async ({params}: {params: Promise<{document_id: string}>}) => {
  const {document_id} = await params;
  return generateMapPageMetadata({searchParams: Promise.resolve({document_id})});
};

export default async function CoiViewPage({params}: {params: Promise<{document_id: string}>}) {
  const {document_id} = await params;
