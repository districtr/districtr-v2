...
2. **Migration safety** — if the diff touches `backend/app/alembic/`, apply
   ...
   [`run-migration`](../run-migration/SKILL.md): drops, type changes
...
3. **Contract drift** — if the diff touches a FastAPI response model,
   ...
   run [`run-api-contract-audit`](../run-api-contract-audit/SKILL.md) and read
   its findings against the diff.

4. **Gate status** — confirm [`run-quality-gate`](../run-quality-gate/SKILL.md)
   has been run for this diff's touched side(s) and is green, per that
   skill's cadence policy.
