...
INTEGRATION_SQLALCHEMY_DATABASE_URI = MultiHostUrl.build(
    scheme=POSTGRES_SCHEME,
    username=POSTGRES_USER,
    host=POSTGRES_SERVER,
    port=int(POSTGRES_PORT),
    path=POSTGRES_INTEGRATION_DB,
    password=POSTGRES_PASSWORD,
)
...
INTEGRATION_OGR2OGR_PG_CONNECTION_STRING = f"PG:host={POSTGRES_SERVER} port={POSTGRES_PORT} dbname={POSTGRES_INTEGRATION_DB} user={POSTGRES_USER} password={POSTGRES_PASSWORD}"

GERRY_DB_FIXTURE_NAME = "ks_demo_view_census_blocks"
ACCOUNT_AUTH0_ID = "auth0|1234567890"
