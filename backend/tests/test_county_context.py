...
the Eguia metric's county ideals and the county_parts forced-minimum precompute.
...
from app.evaluation.splits import county_parts
...
def test_component_populations_for_nodes_single_component():
    ...
    assert component_populations_for_nodes(from_networkx(G), populations) == [600]
...
def test_component_populations_for_nodes_disconnected():
    ...
    result = component_populations_for_nodes(from_networkx(G), populations)
...
# ── component_populations / _ensure_component_populations / _populate_component_populations ──


def test_populate_component_populations_end_to_end(
    session: Session,
    ...
    component_populations is written to the same row total_pop is (keyed by
    gerrydb_table, the parent layer "simple_parent_geos"); the adjacency graph
    is fetched separately via graph_gerrydb_table ("simple_geos", the map's own
    identifier).
    """
    gerrydb_table = "simple_parent_geos"
    graph_gerrydb_table = "simple_geos"
    try:
        COUNTY_CONTEXT._populate_county_data(gerrydb_table, session)
        ...
        COUNTY_CONTEXT._pop_cache.pop(gerrydb_table, None)
        COUNTY_CONTEXT._attempts.pop(gerrydb_table, None)


def test_populate_component_populations_handles_non_integer_total_pop_20(
    session: Session, mock_grid_graph_file
):
    ...
    Reuses the "simple_geos" graph fixture as graph_gerrydb_table, but sources
    population from a standalone synthetic table with a DOUBLE PRECISION
    total_pop_20 column and one NULL row.
    """
    gerrydb_table = "float_pop_test_table"
    graph_gerrydb_table = "simple_geos"
    try:
        session.execute(
            sqlmodel_text(
...
def test_ensure_component_populations_backfills_missing(
    session: Session,
    simple_shatterable_districtr_map,
    gerrydb_simple_geos_view,
    mock_grid_graph_file,
    ...
    try:
        session.execute(
            sqlmodel_text(
                ...
                "VALUES ('00001', :gerrydb_table, 2100, '{}') "
        ...
        COUNTY_CONTEXT._ensure_component_populations(
            gerrydb_table, graph_gerrydb_table, session
        )
...
def test_county_parts_includes_component_populations(
    client,
    session: Session,
    simple_shatterable_districtr_map,
    gerrydb_simple_geos_view,
    ...
    """county_parts()'s returned CountyPartsInfo carries component_populations
    end-to-end (forced-minimum input), independent of parts/total_pop."""
    gerrydb_table = "simple_parent_geos"
    ...
    try:
        ...
        COUNTY_CONTEXT._pop_cache.pop(gerrydb_table, None)
        COUNTY_CONTEXT._component_pop_cache.pop(gerrydb_table, None)
        COUNTY_CONTEXT._attempts.pop(gerrydb_table, None)
