...
def test_create_cms_content(client, mock_session):
    ...
    with patch('uuid.uuid4', return_value=uuid.UUID(test_uuid)):
        content_data = {
            ...
            "draft_content": {"title": "Test Tags", "description": "Test tags description"},
...
def test_create_cms_content_conflict(client, mock_session):
    ...
    content_data = {
        ...
        "slug": "existing-slug",
        "language": LanguageEnum.ENGLISH.value,
        "draft_content": {"title": "Conflict Content", "description": "This should conflict"},
        "published_content": None,
        "districtr_map_slug": "test-map"
    }
...
def test_publish_cms_content(client, mock_session):
    ...
    mock_content = MagicMock()
    mock_content.id = str(uuid.uuid4())
    mock_content.draft_content = {"title": "Draft Content", "description": "Draft description"}
...
def test_list_cms_content_with_language(client, mock_session):
    ...
    response = client.get(
        "/api/cms/content/tags/list?language=en"
    )
...
def test_list_cms_content_pagination(client, mock_session):
    ...
    mock_session.exec.return_value.all.return_value = mock_items[:2]  # Just return first 2 for limit test
    
    response = client.get(
        "/api/cms/content/tags/list?limit=2"
    )
...
def test_get_cms_content_by_slug(client, mock_session):
    ...
    with patch("fastapi.encoders.jsonable_encoder", side_effect=lambda obj, **kwargs: {
        ...
        "draft_content": obj.draft_content if hasattr(obj, "draft_content") else None,
        "published_content": obj.published_content if hasattr(obj, "published_content") else None,
    ...
    } if not isinstance(obj, (list, str, int, float, bool, type(None))) else obj):
        response = client.get(
            "/api/cms/content/tags/slug/test-slug"
        )
...
def test_get_cms_content_by_slug_with_language(client, mock_session):
    ...
    with patch("fastapi.encoders.jsonable_encoder", side_effect=lambda obj, **kwargs: {
        ...
        response = client.get(
            "/api/cms/content/tags/slug/test-slug?language=es"
        )
...
def test_get_cms_content_by_slug_not_found(client, mock_session):
    ...
    response = client.get(
        "/api/cms/content/tags/slug/non-existent-slug"
    )
...
def test_get_cms_content_fallback_language(client, mock_session):
    ...
    with patch("app.cms.main.get_cms_content", side_effect=lambda content_type, slug, language, session: {
        ...
        response = client.get(
            "/api/cms/content/tags/slug/test-slug?language=zh"
        )
