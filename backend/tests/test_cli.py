...
def test_create_overlay(session: Session):
    ...
    stmt = text("SELECT overlay_id, name, description, data_type, layer_type, source FROM overlay WHERE name = :name")
...
def test_create_overlay_with_pmtiles(session: Session):
    ...
    # Verify overlay was created
    stmt = text("SELECT overlay_id, name, data_type, layer_type, source, source_layer FROM overlay WHERE name = :name")
...
def test_create_overlay_with_custom_style(session: Session):
    ...
    result = runner.invoke(
        ...
        [
            ...
            "--data-type", "geojson",
            "--layer-type", "fill",
            "--custom-style", custom_style_json,
        ],
    )
...
def test_create_overlay_and_add_to_map(session: Session, ks_demo_view_census_blocks_districtrmap):
    ...
    result = runner.invoke(
        ...
        [
            ...
            "--data-type", "geojson",
            "--layer-type", "fill",
            "--districtr-map-slug", "ks_demo_view_census_blocks_summary_stats",
        ],
    )
    ...
    map_result = session.execute(stmt, {"slug": "ks_demo_view_census_blocks_summary_stats"}).one_or_none()
...
def test_update_overlay(session: Session):
    ...
    stmt = text("SELECT name, description, layer_type FROM overlay WHERE overlay_id = :overlay_id")
