import pytest
from sqlmodel import Session, col, select, insert
...
class TestCommentListEndpoints:
    ...
    @patch("app.comments.moderation.score_text", return_value=TEST_MODERATION_SCORE)
    def test_admin_list_tag_arrays_stay_aligned(
        self, mock_score_text, client, session: Session
    ):
        """tags / tag_ids / tag_review_status / tag_moderation_score are read as
        PARALLEL arrays (the CMS moderation UI zips them into per-tag review
        controls), so they must share one ordering. array_agg(DISTINCT slug)
        used to sort alphabetically while the id/status aggregates kept join
        order — pairing a slug with another tag's id. Tags are submitted here in
        an order that is NOT alphabetical so a regression re-misaligns them."""
        form_data = {
            "commenter": {"first_name": "Align", "email": "align@example.com"},
            "comment": {"title": "Aligned", "comment": "Tag ordering matters."},
            "tags": [{"tag": "zebra"}, {"tag": "alpha"}],
            "turnstile_token": "test_token",
        }
        assert client.post("/api/comments/submit", json=form_data).status_code == 201

        response = client.get("/api/comments/admin/list")
        assert response.status_code == 200
        entry = response.json()[0]
        assert sorted(entry["tags"]) == ["alpha", "zebra"]

        id_by_slug = {
            row.slug: row.id
            for row in session.exec(
                select(Tag).where(col(Tag.slug).in_(["alpha", "zebra"]))
            ).all()
        }
        # Position-by-position: slug i must carry slug i's own id.
        assert [id_by_slug[slug] for slug in entry["tags"]] == entry["tag_ids"]
        assert len(entry["tag_review_status"]) == len(entry["tags"])
        assert len(entry["tag_moderation_score"]) == len(entry["tags"])
