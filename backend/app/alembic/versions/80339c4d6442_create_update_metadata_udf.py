...
def upgrade() -> None:
    ...
    op.execute(
        sa.text(
            """
            CREATE OR REPLACE FUNCTION document.update_metadata(metadata JSONB)
            RETURNS VOID AS $$
            DECLARE
                meta JSONB;
            BEGIN
                ...
                LOOP
                    RAISE NOTICE 'Processing document_id: %', meta->>'document_id';
                    IF EXISTS (
                        SELECT 1
                        FROM document.document_metadata
                        WHERE document_metadata.document_id = meta->>'document_id'
                    ...
                    ELSE
                        ...
                        VALUES ((meta->>'document_id')::UUID, meta->>'key', meta->>'value', NOW(), NOW());
                    END IF;
                END LOOP;
            END;
            $$ LANGUAGE plpgsql;
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.execute("DROP FUNCTION document.update_metadata(metadata JSONB)")
    # ### end Alembic commands ###
