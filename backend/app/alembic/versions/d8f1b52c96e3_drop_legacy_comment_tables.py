...
Legacy form comments are dropped, not converted. Production held five, four
of them test rows, and none of the legacy portals drew real use, so a
converter would mostly exist to carry test data forward while carrying its
own edge cases. Take a pg_dump of the comments schema before running this
(the cutover runbook's step 1) and backfill from it if any row is ever
wanted. Legacy portal pages keep their form blocks; with no form config
those render nothing until an admin attaches a form in the Portals admin.

Also adds submissions.map_is_clone: true for submission-owned frozen clones
(every row submitted before this migration was cloned at submission), false
for live references to an author's working map (drafts, auto-collect
modes). Takedown may only demote the draft_status of clones.

Negative-zone rows (skipped by b3d9f47a25c1 as never-renderable) are dropped
with the tables.
...
def upgrade() -> None:
    ...
    # document (drafts, later auto-collect modes). Takedown may only demote
    # the draft_status of CLONES.
    op.add_column(
        "submissions",
        sa.Column(
            "map_is_clone",
