...
from app.models import CMSContent, CMSContentCreate, CMSContentUpdate, CMSContentPublic, LanguageEnum
...
@router.post("/content", response_model=CMSContentPublic, status_code=status.HTTP_201_CREATED)
async def create_cms_content(data: CMSContentCreate, session: Session = Depends(get_session)):
...
async def update_cms_content(
    ...
    if (data.slug and data.slug != content.slug) or (data.language and data.language != content.language):
