...
backend computes for the document evaluation table. Please update ``METRICS'' when adding or removing a metric.

``CURRENT_PAYLOAD_VERSION`` is a deterministic 63-bit integer derived
from the SHA-256 of ``METRICS`` in canonical sorted order.  It should be stamped
into ``document.evaluation.payload_version`` on every write.  An evaluation
row whose stored version differs from this value is treated as stale
and recomputed on the next read.

``CURRENT_PAYLOAD_VERSION`` flips when:

- a metric is added or removed (the key set changes),
- a metric's per-metric ``version`` is bumped, when we want to change a formula or output shape in a way that
