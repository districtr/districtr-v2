...
Operator checklist for the production run (`STRESS_TEST_PLAN.md` §5 WS5).
...
- [ ] **Deploy observability** (WS3 deferred): `cd infra && pulumi preview --stack prod`
...
- [ ] **CDN config live**: `curl -sf https://tilesets1.cdn.districtr.org/stress-test/config.json`
      returns the plan-§8 JSON (was 403 during build).
...
- [ ] **`SCALE=0.01` dry run green, same day** (WS4 deferred acceptance):
...
- [ ] Open CloudWatch dashboard **`districtr-prod-stress-test`** and let it
      ...
      DatabaseConnections. Screenshot for the WS6 report.
...
- [ ] Tail progress: `aws ssm start-session --target $INSTANCE_ID`, then
      ...
      single locust process pins a core, note it for WS6 (`--processes 4` next
      time).
