...
- **`BlockAssignmentsCSV`** (`build_block_assignments_csv`) — two columns, `geo_id,zone`, one row per assigned block. If the map has a `child_layer` (shatterable), rows are expanded through the contiguity graph (`get_graph(gerrydb_table_name)`): a parent assignment is expanded to one row per child via `G.nodes[geo_id]["children"]`, so every row in the export is always at the finest (block) granularity regardless of what the user actually painted at. This is why the export path depends on `learn-map-data`'s graph availability invariant — a missing graph npz for a shatterable map breaks this export, not just contiguity checks.
