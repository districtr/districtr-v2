...
Edit the model in `backend/app/models.py` (or `app/submissions/models.py`, `app/district_notes/models.py`, etc.), then:

```bash
docker-compose exec backend alembic revision --autogenerate -m "<description>"
```

`env.py`'s `include_object` hook excludes three schemas from autogenerate diffing entirely. `gerrydb` holds imported layers, owned by the import pipeline. `admin` holds the Wagtail CMS tables, owned by Django migrations under `cms/`. `cms` holds the legacy content tables and `site_settings`, so an edit to `app/cms/models.py` needs a hand-written migration. A model added in any of these schemas will never autogenerate a migration, and that's the guard working, not a bug to route around. It also excludes tables by name: entries in `POST_GIS_ALPINE_RESERVED_TABLES` (`backend/app/alembic/constants.py`), `parentchildedges_*` partitions, and `*_districtr_view` materialized views.
